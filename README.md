...
- **Better Matching** -- Include mutiple options for metadata services with potential to use multiple results in matching logic
- **Docker container** -- Headless server mode with a web UI, no Electron dependency
- **Audiobook support** -- Match against OpenLibrary/Audible, name for Audiobookshelf
- **Music support** -- Match against MusicBrainz/TADB, name schemes for Navidrome/Jellyfin
- **Rust port (?)** -- Rewrite the core scanner, matcher, and transfer engine in Rust for native performance and a smaller footprint. POC already in the works.
- **Torrent monitoring/integration** -- Watch a download directory and auto-ingest completed downloads

## Help Wanted

ReelName is primarily developed and tested on Windows. The macOS and Linux builds are generated by CI but have not been thoroughly tested. If you're running either platform and willing to try things out, bug reports and feedback are very welcome — open an [issue](https://github.com/sentient-soup/reelname/issues) with your OS, version, and what broke.

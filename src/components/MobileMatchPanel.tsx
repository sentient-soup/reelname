...
/**
 * Mobile-only fullscreen match panel.
 * Rendered outside overflow-hidden containers in page.tsx so z-index works reliably.
 * Only visible on screens < md (768px).
 */
export function MobileMatchPanel({ onRefresh }: { onRefresh: () => void }) {
  const {
    activeGroup,
    matchPanelOpen,
    setMatchPanelOpen,
  ...
  // Only show on mobile when a group is selected
  if (!matchPanelOpen || !activeGroup) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-bg-secondary md:hidden">
      {/* Header */}

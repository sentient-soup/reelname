      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {Object.keys(selectedGroupIds).length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => handleBulk("confirm")} className="px-2.5 py-1 text-xs rounded bg-status-confirmed text-white hover:opacity-90 transition-opacity">Confirm</button>
            <button onClick={() => handleBulk("skip")} className="px-2.5 py-1 text-xs rounded bg-status-skipped text-white hover:opacity-90 transition-opacity">Skip</button>
            <button onClick={() => handleBulk("rematch")} className="px-2.5 py-1 text-xs rounded bg-status-ambiguous text-white hover:opacity-90 transition-opacity">Rematch</button>
            <button onClick={() => handleBulk("delete")} className="px-2.5 py-1 text-xs rounded bg-status-failed text-white hover:opacity-90 transition-opacity">Delete</button>
            <button onClick={clearSelection} className="px-2.5 py-1 text-xs rounded bg-bg-tertiary text-text-secondary hover:bg-bg-hover transition-colors">Clear</button>
          </div>
        )}
      </div>
    </div>

...
function useBreakpoint(): Breakpoint {
  ...
  useEffect(() => {
    ...
    const update = () => {
      if (xlMql.matches) setBp("xl");
      else if (mdMql.matches) setBp("md");
      else setBp("mobile");
    };
    update();
...
export function MatchPanel({ onRefresh }: { onRefresh: () => void }) {
  ...
  const containerClass = isMobile
    ...
    : "flex flex-col border-l border-border bg-bg-secondary h-full overflow-hidden";
